...
`Capture -> Segment -> Recognize -> Normalize -> Translate(optional, independent provider) -> Persist -> Deliver`
...
这里的 `translation` 指“引擎原生翻译能力”，不是“整个产品是否可翻译”。

V2 首选模型是：

1. ASR profile 决定“怎么转录”
2. translation provider 决定“是否翻译、翻到哪种语言”
3. 二者允许交叉组合，例如“本地转录 + 云翻译”
...
翻译配置必须独立于 `Speech Engine` 预设存在，不要求识别引擎自己支持翻译。
